<script lang="ts">
	...
	const logIn = async () => {
		try {
			const userData = await pb.collection('users').authWithPassword('test@dev.com', 'netapp123');
...
<Heading class="p-8" tag="h1" customSize="text-3xl">Welcome to {APP_NAME}</Heading>
